...
describe('Electrum', () => {
  it('ElectrumClient can estimate fees from histogram', async () => {
    assert.strictEqual(
      Electrum.calcEstimateFeeFromFeeHistorgam(1, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      Electrum.calcEstimateFeeFromFeeHistorgam(18, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      Electrum.calcEstimateFeeFromFeeHistorgam(144, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
  ...
  it('ElectrumClient can test connection', async () => {
    assert.ok(!(await Electrum.testConnection('electrum1.bluewallet.io', 444, false)));
    assert.ok(!(await Electrum.testConnection('electrum1.bluewallet.io', false, 444)));
    assert.ok(!(await Electrum.testConnection('ya.ru', 444, false)));
    assert.ok(!(await Electrum.testConnection('google.com', false, 80)));
    assert.ok(!(await Electrum.testConnection('google.com', 80, false)));
    assert.ok(!(await Electrum.testConnection('google.com', false, 443)));
    assert.ok(!(await Electrum.testConnection('google.com', 443, false)));
    assert.ok(!(await Electrum.testConnection('joyreactor.cc', false, 443)));
    assert.ok(!(await Electrum.testConnection('joyreactor.cc', 443, false)));
  ...
  it('ElectrumClient can estimate fees', async () => {
    assert.ok((await Electrum.estimateFee(1)) >= 1);
  ...
  it('Electrum can do getBalanceByAddress()', async function () {
    const address = '3GCvDBAktgQQtsbN6x5DYiQCMmgZ9Yk8BK';
    const balance = await Electrum.getBalanceByAddress(address);
  ...
  it('Electrum can do getTransactionsByAddress()', async function () {
    const txs = await Electrum.getTransactionsByAddress('bc1qe7q08prc2spln2l7qdvvlcgqxm9za9z7mjnpzc');
  ...
  it.skip('Electrum can do getMempoolTransactionsByAddress()', async function () {
    const txs = await Electrum.getMempoolTransactionsByAddress('bc1qp33en9mnw277c9vz5fz9vcu666cvervdnk02327wwph97hdjurqqxtl03c');
    assert.ok(txs.length > 0);
    assert.ok(txs[0].tx_hash);
    assert.ok(txs[0].fee);

    const rez = await Electrum.multiGetTransactionByTxid([txs[0].tx_hash], 10, true);
    assert.ok(rez[txs[0].tx_hash]);
  });

  it('Electrum can do getTransactionsFullByAddress()', async function () {
    const txs = await Electrum.getTransactionsFullByAddress('bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh');
  ...
  it('Electrum can do txhexToElectrumTransaction()', () => {
    const tx =
    ...
    const decoded = Electrum.txhexToElectrumTransaction(tx);
  ...
  it.each([false, true])('Electrum can do multiGetBalanceByAddress(), disableBatching=%p', async function (diableBatching) {
    if (diableBatching) Electrum.setBatchingDisabled();
    const balances = await Electrum.multiGetBalanceByAddress([
    ...
    if (diableBatching) Electrum.setBatchingEnabled();
  });

  it('Electrum can do multiGetUtxoByAddress()', async () => {
    const utxos = await Electrum.multiGetUtxoByAddress(
      [
        'bc1qe7q08prc2spln2l7qdvvlcgqxm9za9z7mjnpzc',
        'bc1qdsf8p4knu2u0h9cspflh0ftjp8qayve3r5nme8',
        'bc1qzfmm8d9saalnjjnskj2mekycg73hecpajt8urp',
  ...
  it.each([false, true])('ElectrumClient can do multiGetHistoryByAddress(), disableBatching=%p', async disableBatching => {
    if (disableBatching) Electrum.setBatchingDisabled();
    const histories = await Electrum.multiGetHistoryByAddress(
      [
    ...
    if (disableBatching) Electrum.setBatchingEnabled();
  });
