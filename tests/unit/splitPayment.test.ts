...
describe('deRound', () => {
  ...
  it('draws the nudge from two bytes, so deltas beyond 256 are reachable', async () => {
    // all-0xff rng -> delta = 1 + (0xffff % 999) = 601; a single-byte draw caps at 256
    const out = await deRound([100_000, 73_321], 25_000, fixedRng(0xff));
...
describe('planChangeOutputs', () => {
  ...
  it('retains change outputs exactly at the dust boundary (330 sats)', async () => {
    const out = await planChangeOutputs({
      ...common,
      change: 330,
      pMax: 80_000,
...
describe('planSplitOutputs', () => {
  ...
  it('caps change outputs at MAX_CHANGE_OUTPUTS', async () => {
    const { changeAmounts } = await planSplitOutputs({
      paymentValue: 300_000,
  ...
  it('never burns change: when it splits, a change output survives above dust', async () => {
    for (let trial = 0; trial < 30; trial++) {
      const { paymentAmounts, changeAmounts } = await planSplitOutputs({
        paymentValue: 500_000,
        changeValue: 5_000,
  ...
  it('never burns change for a pinned split whose change budget has since shrunk', async () => {
  ...
  it('honours a valid pin when the change budget does allow the split', async () => {
...
describe('partitionPaymentAmounts', () => {
  it('returns n parts summing to value', async () => {
    const amounts = await partitionPaymentAmounts(500_000, 2, 1);
    expect(amounts).toHaveLength(2);
    expect(amounts.reduce((a, b) => a + b, 0)).toBe(500_000);
  });
  it('returns non-round amounts (de-rounded)', async () => {
    let allNonRound = true;
    for (let trial = 0; trial < 20; trial++) {
      const amounts = await partitionPaymentAmounts(400_000, 2, 3);
